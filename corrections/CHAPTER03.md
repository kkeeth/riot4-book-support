...
本チャプターの誤字・脱字・誤植がかなり多く、ご購入いただいた皆さんには大変ご迷惑をおかけしてしまい、誠に申し訳ございません。

つきましては、[本チャプターのソースコードのサンプル](https://github.com/kkeeth/riot4-book-example-apps/tree/master/CHAPTER3) につきましては、最後の完成コードのみではなく、チャプター内のセクションごとのソースコードも用意いたしましたので、こちらも見ていただくと理解しやすいと思いますのでご参照いただけますと幸いです。
...
三点目：
大変に申し訳ないですが、本文中の `⑤では、〜` の途中からの文言が全て誤植となります。。。
